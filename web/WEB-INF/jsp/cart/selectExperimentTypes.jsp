...
<script language="Javascript">
	...
	function decodeExperimentName(encoding) {
	...
	function synchronize(encoding) {
		var expName = decodeExperimentName(encoding);
		...
		for (var i = 0; i < elements.length; i++) {
			...
			if (name.indexOf(encoding) == 0)
				...
				var id = decodeExperimentName(name);
				if (id == expName)
					element.value = value;
			}
		}
...
<html:form action="/cart/recordSelectedExperimentTypes">
			...
			<!-- Experiment -->
       		<tr class="<%=style[styleIdx++ % 2]%>">
        		<td>
        			...
        			<select name="<%= expKey %>" onchange="synchronize('<%= expKey %>')">
        				<option value="none">Select experiment type</option>
        				<logic:iterate id="bioAssayType" name="bioAssayTypes">
        					<option value="<bean:write name="bioAssayType" property="id"/>">
        						<bean:write name="bioAssayType" property="name"/>
        					</option>
        				</logic:iterate>
         			</select>
        		</td>
